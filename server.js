...
app.post("/login",(req,res)=>{
  ...
  let hasArrayWithHash = userInfo.some(userInfo => userInfo.password === hash);
  if (hasArrayWithEmail || hasArrayWithHash) {
...
app.post("/signup",(req,res) =>{
    ...
    const birthYears = req.body.birthYear;
    ...
    // console.log(hash);
    // 여기서 이메일, 패스워드 DB에 넣고
    let hasArrayWithEmail = userInfo.some(userInfo => userInfo.email === emails);
    if (hasArrayWithEmail) {
      res.json({result:'already'});
    } else {
    let personInfo = {email:emails, password: hash,birthYear:birthYears, gender:genders};
